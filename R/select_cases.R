...
#' @param id optional unit id for panels
#' @param time optional time variable for panels
...
#' @param ... reserved
#' @return csel object with results, spec, model (if any), distances (if any), audit_log
#' @references
#'   Gerring & Cojocaru (2016) Table 1 (case-selection goals/strategies) and the 7-step algorithmic protocol.
#'   Seawright & Gerring (2008) quantitative procedures for typical, diverse, extreme, deviant,
#'   influential, most-similar, and most-different strategies.
#' @export
select_cases <- function(
  ...
  y <- f$y; x <- f$x; z <- f$z
  all_vars <- unique(c(y, x, z, id, time))
  missing_vars <- setdiff(all_vars, names(data))
  if (length(missing_vars)) stop("Missing variables in `data`: ", paste(missing_vars, collapse = ", "))

  spec <- list(
    y = y, x = x, z = z, id = if (!missing(id)) deparse(substitute(id)) else NULL,
    time = if (!missing(time)) deparse(substitute(time)) else NULL,
  ...
  out <- switch(
    ...
    index          = strategy_index(data, y, id, time, n, audit, spec),
    longitudinal   = strategy_longitudinal(data, y, x, z, id, time, n, audit, spec),
