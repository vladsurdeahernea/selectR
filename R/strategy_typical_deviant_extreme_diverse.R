...
strategy_typical <- function(data, y, x, z, model, family, n, audit, spec) {
  ...
  audit <- c(audit, mk_audit(5, "Fitted model for residuals."),
             mk_audit(6, "Ranked by smallest |residual| (typical)."))
  keep <- c(names(data), ".resid", ".hat", ".cook", "score", "why")
  new_csel(utils::head(ord[, keep, drop = FALSE], n), spec, model = m, distances = NULL, audit = audit)
}
...
strategy_deviant <- function(data, y, x, z, model, family, n, audit, spec) {
  ...
  ord <- df[order(-df$score), , drop = FALSE]
  audit <- c(audit, mk_audit(5, "Fitted model for residuals."),
             mk_audit(6, "Ranked by largest |residual| (deviant)."))
  keep <- c(names(data), ".resid", ".hat", ".cook", "score", "why")
  new_csel(utils::head(ord[, keep, drop = FALSE], n), spec, model = m, distances = NULL, audit = audit)
}
